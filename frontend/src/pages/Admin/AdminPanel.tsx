...
//   /order_details, /employee_orders (по имени), /admin_send_label (по имени),
...
export const AdminPanel = () => {
  ...
  // === /employee_orders <имя_сотрудника> ===
  const handleLoadEmployeeOrders = async () => {
    if (!ordersUserId) return;
    setOrdersLoading(true);
    try {
      const data = await adminApi.getUserOrders(parseInt(ordersUserId, 10));
      setEmployeeOrders(data || []);
      setOrdersLoadedName(employeeName(employees, ordersUserId));
    } catch (err: any) {
      toast.error(
        err.response?.data?.error ||
          err.message ||
          "Не удалось загрузить заказы сотрудника",
      );
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader>
            <CardTitle>📋 Активные заказы сотрудника</CardTitle>
            <CardDescription>
              Аналог /employee_orders: выберите сотрудника по имени
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex flex-wrap items-end gap-2">
              <div className="w-64 space-y-2">
                ...
                <div className="flex gap-2">
                  ...
                  <Select
                    value={ordersUserId}
                    onValueChange={(v) => setOrdersUserId(v ?? "")}
                  >
                    <SelectTrigger>
                  ...
                  <Button
                    onClick={handleLoadEmployeeOrders}
                    disabled={ordersLoading || !ordersUserId}
                  >
                    {ordersLoading ? "Загрузка..." : "Показать"}
                  </Button>
                </div>
              </div>
            </div>
            ...
            {ordersLoadedName && (
              <div className="rounded-md bg-muted/50 p-3 text-sm">
                <p className="mb-2">
                  Активные заказы <b>{ordersLoadedName}</b>:{" "}
                  {employeeOrders.length === 0 ? "нет" : employeeOrders.length}
                </p>
                  ...
                  <ul className="space-y-1">
                    {employeeOrders.map((o) => (
                      <li key={o.order_id}>
                        ...
                        <span className="text-muted-foreground">
                          (назначен {formatDateTime(o.assigned_at)})
                        </span>
                      </li>
                    ))}
                  </ul>
